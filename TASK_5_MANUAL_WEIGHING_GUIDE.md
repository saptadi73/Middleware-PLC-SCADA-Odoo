...
### Memory Allocation (D9000-D9013)

TASK 5 uses a **dedicated memory area separate from Equipment Failure monitoring** to avoid conflicts.

```
Memory Area: D9000-D9013 (14 words total)
───────────────────────────────────────────
D9000       : BATCH (REAL, scale=1)
D9001-D9008 : NO-MO (ASCII, 8 chars, 4 words)
D9009-D9010 : NO-Product (REAL, 2 words, scale=1)
D9011-D9012 : Consumption (REAL, 2 words, scale=100)
D9013       : status_manual_weigh_read (BOOLEAN handshake flag)
───────────────────────────────────────────
```
...
- **Manual Weighing (D9000-D9013)**: Operator-entered consumption data
...
| NO-Product | D9009-D9010 | REAL | 1 | Product Template ID in Odoo (2 words) | 23 |
| Consumption | D9011-D9012 | REAL | 100 | Material consumption quantity (scaled, 2 words) | 82500 → 825 kg |
| Handshake | D9013 | BOOLEAN | - | Read status flag (0=new, 1=read) | 0 or 1 |

### Scale Factor Details

**Consumption (D9011-D9012)** uses scale factor of 100:
...
│   - Read D9000-D9013 (14 words)      │
│   - Check D9013 (handshake flag)     │
│   - Skip if D9013=1 (already read)   │
└──────────────────────────────────────┘
            ↓
┌──────────────────────────────────────┐
...
└──────────────────────────────────────┘
            ↓
    Validation Passed?
   ├─ NO  → Log error, keep D9013=0 (retry next cycle)
...
│   - Set D9013 = 1 (mark as read)     │
│   - PLC receives signal at next read │
│   - PLC resets D9013 = 0 on new msg  │
└──────────────────────────────────────┘
            ↓
┌──────────────────────────────────────┐
...
- [x] Create ADDITIONAL_EQUIPMENT_REFERENCE.json (updated with D9000-D9013, not D8000)
- [x] Update konsep_task.txt with TASK 5 definition
- [x] Add D9000-D9013 to MEMORY ARCHITECTURE section
...
- [x] Update plc_handshake_service.py with D9013 support
   - [x] Add MANUAL_WEIGHING_STATUS_ADDRESS = 9013
...
1. **read_manual_weighing_data() → Dict**
   - Reads D9000-D9012 from PLC
   - Parses BATCH, MO-ID, Product, Consumption
   - Checks D9013 handshake (skips if already read)
...
4. **mark_handshake() → bool**
   - Sets D9013 = 1 via handshake service
...
Added methods for D9013 (manual weighing handshake):
...
|-------|--------|-----------|
| MO ID empty | Log error, don't sync | Retry (D9013 = 0) |
| Product not found | Log error, don't sync | Retry (D9013 = 0) |
| Consumption ≤ 0 | Log error, don't sync | Retry (D9013 = 0) |

### Sync Errors

| Error | Action | Next Cycle |
|-------|--------|-----------|
| Network timeout | Log error, don't mark | Retry (D9013 = 0) |
| MO not found in Odoo | Log error, don't mark | Retry (D9013 = 0) |
| Invalid product | Log error, don't mark | Retry (D9013 = 0) |
| Odoo error 500 | Log error, don't mark | Retry (D9013 = 0) |

### Retry Logic

- Keep D9013 = 0 on any failure
...
1. **Simulate PLC Input** (direct memory write or HMI):
   ...
   D9009 = 45 (product ID)
   D9011-D9012 = 100000 (1000 kg * 100)
   D9013 = 0 (new data)
   ```
...
4. **Verify handshake**:
   - Read D9013, should be 1 if sync successful
   - Next PLC cycle should reset D9013 = 0
...
[INFO] Marked D9013 (manual weighing read) as read
...
| D9013 stuck at 1 | Handshake not resetting | Check PLC logic, manually reset via test script |
...
- ✅ Allocated memory D9000-D9013 (separate from equipment failure D8000-D8022)
...
- ✅ Updated plc_handshake_service.py with D9013 support
